...
assert.match(app, /OVERLAY_CHARACTER === "maho"/);
assert.match(app, /if \(OVERLAY_CHARACTER === "maho"\) return/);
assert.doesNotMatch(app, /Message Amadeus/);

const character = await readFile(new URL("../src/character.ts", import.meta.url), "utf8");
